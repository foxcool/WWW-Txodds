# This Makefile is for the WWW::Txodds extension to perl.
#
...
#     ABSTRACT_FROM => q[lib/WWW/Txodds.pm]
#     AUTHOR => [q["Foxcool" <"">]]
#     BUILD_REQUIRES => {  }
#     CONFIGURE_REQUIRES => {  }
#     LICENSE => q[perl]
...
#     VERSION_FROM => q[lib/WWW/Txodds.pm]
#     clean => { FILES=>q[Txodds-*] }
...
TO_INST_PM = lib/WWW/Txodds.pm

PM_TO_BLIB = lib/WWW/Txodds.pm \
...
manifypods : pure_all  \
	...
	$(NOECHO) $(POD2MAN) --section=3 --perm_rw=$(PERM_RW) \
	  lib/WWW/Txodds.pm $(INST_MAN3DIR)/WWW::Txodds.$(MAN3EXT) 
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'abstract: '\''TXOdds.com API Perl interface.'\''' >> META_new.yml
	$(NOECHO) $(ECHO) 'author:' >> META_new.yml
	$(NOECHO) $(ECHO) '  - "\"Foxcool\" <\"\">"' >> META_new.yml
	...
	$(NOECHO) $(ECHO) 'name: WWW-Txodds' >> META_new.yml
	...
	$(NOECHO) $(ECHO) 'version: 0.66' >> META_new.yml
	-$(NOECHO) $(MV) META_new.yml $(DISTVNAME)/META.yml
	$(NOECHO) $(ECHO) Generating META.json
	$(NOECHO) $(ECHO) '{' > META_new.json
	$(NOECHO) $(ECHO) '   "abstract" : "TXOdds.com API Perl interface.",' >> META_new.json
	$(NOECHO) $(ECHO) '   "author" : [' >> META_new.json
	$(NOECHO) $(ECHO) '      "\"Foxcool\" <\"\">"' >> META_new.json
	$(NOECHO) $(ECHO) '   ],' >> META_new.json
	...
	$(NOECHO) $(ECHO) '   },' >> META_new.json
	$(NOECHO) $(ECHO) '   "name" : "WWW-Txodds",' >> META_new.json
	...
	$(NOECHO) $(ECHO) '   "version" : "0.66"' >> META_new.json
	$(NOECHO) $(ECHO) '}' >> META_new.json
...
ppd :
	$(NOECHO) $(ECHO) '<SOFTPKG NAME="$(DISTNAME)" VERSION="0.66">' > $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <ABSTRACT>TXOdds.com API Perl interface.</ABSTRACT>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>"Foxcool" &lt;""&gt;</AUTHOR>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <IMPLEMENTATION>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Test::More" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <ARCHITECTURE NAME="i386-linux-thread-multi-5.12" />' >> $(DISTNAME).ppd
...
pm_to_blib : $(FIRST_MAKEFILE) $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', q[$(PM_FILTER)], '\''$(PERM_DIR)'\'')' -- \
	  lib/WWW/Txodds.pm blib/lib/WWW/Txodds.pm 
